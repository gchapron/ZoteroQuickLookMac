...
Build with `./build.sh`, install `zoteroquicklookmac-1.0.11.xpi` in Zotero, and restart Zotero. These checks require a real Zotero library and the installed plugin; successful compilation alone does not verify them.
...
Record the macOS and Zotero versions used and any failures. Building the package does not publish it or modify the stable update feed.
